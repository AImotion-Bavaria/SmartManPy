...
from manpy.simulation import LineGenerationJSON
import json
import os
import glob
import tempfile
  ...
  """
    ...
    def checkTopology(self, filename=None):
        file_path = os.path.join(
            project_path, "manpy", "simulation", "JSONInputs", filename
        )
...
for filepath in glob.glob(
    os.path.join(project_path, "manpy", "simulation", "JSONInputs", "*.json")
):
    ...
    setattr(
        SimulationTopology, "test_%s" % filename, getTestTopology(filename=filename)
    )
