...
class MachineManagedJob(MachineJobShop):
    ...
    def canAccept(self, callerObject=None):
        ...
        return (
            len(activeObjectQueue) < activeObject.capacity
    ...
    def canAcceptAndIsRequested(self, callerObject=None):
        ...
        if activeObject.operatorPool != "None" and (
            any(type == "Load" for type in activeObject.multOperationTypeList)
            or any(type == "Setup" for type in activeObject.multOperationTypeList)
        ):
            if giverObject.haveToDispose(activeObject):
                if (
                    activeObject.checkIfActive()
        ...
        elif activeObject.operatorPool != "None" and any(
            type == "Processing" for type in activeObject.multOperationTypeList
        ):
            if giverObject.haveToDispose(activeObject):
    ...
    def checkOperator(self, callerObject=None):  # , candidateEntity=None):
        assert (
            callerObject != None
        ), "checkOperator must have a caller for MachineManagedJob"
        activeObject = self.getActiveObject()
        #         giverObject=activeObject.getGiverObject()
        giverObject = callerObject
        giverObjectQueue = giverObject.getActiveObjectQueue()
