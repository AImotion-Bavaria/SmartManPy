...
class Job(Entity):  # inherits from the Entity class
    ...
    def __init__(
        ...
        **kw
        ...
        if self.remainingRoute:
            ...
            initialSetupType = (
                self.remainingRoute[0].get("operationType", {}).get("Setup", 0)
            )
            initialProcessingType = (
                self.remainingRoute[0].get("operationType", {}).get("Processing", 0)
            )
            #             initialSetupType=0
            #             if initialSetup:
            ...
            self.initialOperationTypes = {
                "Setup": initialSetupType,
        ...
        if (
            ...
            if self.schedule:
                ...
                if self.schedule[-1]["station"].type == "Exit":
                    json["results"]["completionTime"] = self.schedule[-1][
                        "entranceTime"
                    ]
                ...
                # XXX now Orders do not run through the system but OrderDesigns do
                    ...
                    self.type == "OrderDesign"
                    and self.schedule[-1]["station"].type == "OrderDecomposition"
                ):  #
                    json["results"]["completionTime"] = self.schedule[-1][
                        "entranceTime"
                    ]
                ...
                elif self.type == "OrderComponent" and self.schedule[-1][
                    "station"
                ].__class__.__name__ in set(["MouldAssemblyManaged", "MouldAssembly"]):
                    json["results"]["completionTime"] = self.schedule[-1][
                        "entranceTime"
                    ]
                    completionTime = self.schedule[-1]["entranceTime"]
                ...
                for record in self.schedule:
                    ...
                    json["results"]["schedule"][i]["stationId"] = record[
                        "station"
                    ].id  # id of the Object
                    json["results"]["schedule"][i]["entranceTime"] = record[
                        "entranceTime"
        ...
        if self.currentStation and self.routeInBOM:
            ...
            for step in self.route:
                ...
                if self.currentStation.id in stepObjectIds:
                    ...
                    assert (
                        self.currentStation.id in firstStepStationIdsList
                    ), "the initialStation is not in the first step's stationIdsList"
    ...
    def findCandidateReceiver(self):
        ...
        availableReceivers = [
            x for x in self.candidateReceivers if not x in router.occupiedReceivers
        ]
        ...
        if not availableReceiver and bool(availableReceivers):
            availableReceiver = self.currentStation.selectReceiver(
                        ...
                        for cell in G.cells_to_write:
                            if cell["row"] == step + 1 and cell["col"] == col_to_write:
                                ...
                                if not next_col in [
                                    ...
                                    G.routeTraceSheet.write(
                                        step + 1, next_col, self.alias
                                    )  # XXX
                                    stepDone = True  # XXX
                                    break  # XXX
                                cell["job"] = cell["job"] + "," + self.alias
                                G.routeTraceSheet.write(
                                    cell["row"], cell["col"], cell["job"]
                                )
                                stepDone = True
                                break
                        if not stepDone:
