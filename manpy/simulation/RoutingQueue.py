...
class RoutingQueue(Queue):
                ...
                if not entity.receiver:
                    ...
                    for o in self.next:
                        if o.getActiveObjectQueue():
                            if o.getActiveObjectQueue()[
                                0
                            ].parentBatch == entity.parentBatch and not (
                                o is callerObject
                            ):
                                return False
                    isInRouting = True
                    break
        return (
            len(activeObjectQueue) > 0 and thecaller.isInRouteOf(self) and isInRouting
        )
    ...
    def sortEntitiesForReceiver(self, receiver=None):
        ...
        activeObjectQueue.sort(
            key=lambda x: not (x.receiver is receiver), reverse=False
        )
