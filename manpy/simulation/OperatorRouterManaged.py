...
class RouterManaged(Router):
        ...
        for operator in [x for x in self.candidateOperators if x.isAssignedTo()]:
            if not operator.isAssignedTo() in self.pendingObjects:
                if operator.candidateEntity.currentStation.exitIsAssignedTo():
                    if (
                        operator.isAssignedTo()
                        != operator.candidateEntity.currentStation.exitIsAssignedTo()
                    ):
        ...
        for operator in self.candidateOperators:
            ...
            if operator.checkIfResourceIsAvailable():
                if operator.candidateEntity:
                    ...
                    if (
                        (operator.candidateEntity.currentStation in self.pendingObjects)
                        ...
                        self.printTrace(
                            ...
                            + operator.id
                            + " to -->  "
                            + operator.candidateEntity.candidateReceiver.id,
                        )
                        operator.assignTo(operator.candidateEntity.candidateReceiver)
                        if (
                            not operator.candidateEntity.currentStation
                            in self.toBeSignalled
                        ):
                            self.toBeSignalled.append(
                                operator.candidateEntity.currentStation
                            )
        self.printTrace(
            "objects to be signalled:" + " " * 11,
        ...
        for operator in self.candidateOperators:
            ...
            if station:
                ...
                if station in self.pendingMachines and station in self.toBeSignalled:
                    ...
                    self.printTrace(
                        "router",
                        "signalling broker of" + " " * 50 + operator.isAssignedTo().id,
                    )
                    if operator.isAssignedTo().broker.expectedSignals[
                        "resourceAvailable"
                    ]:
                        self.sendSignal(
                            receiver=operator.isAssignedTo().broker,
                            signal=operator.isAssignedTo().broker.resourceAvailable,
                        )
                elif (not station in self.pendingMachines) or (
                    not station in self.toBeSignalled
                ):
                    # signal the queue proceeding the station
                    assert (
                        operator.candidateEntity.currentStation in self.toBeSignalled
                    ), "the candidateEntity currentStation is not picked by the Router"
                    assert (
                        operator.candidateEntity.currentStation in G.QueueList
                    ), "the candidateEntity currentStation to receive signal from Router is not a queue"
                    if operator.candidateEntity.candidateReceiver.canAccept() and any(
                        type == "Load"
                        for type in operator.candidateEntity.candidateReceiver.multOperationTypeList
                    ):
                        # if the station is already is already signalled then do not send event
                        if (
                            not operator.candidateEntity.currentStation.loadOperatorAvailable.triggered
                        ):
                            self.printTrace(
                                "router",
                                "signalling queue"
                                + " " * 50
                                + operator.candidateEntity.currentStation.id,
                            )
                            if operator.candidateEntity.currentStation.expectedSignals[
                                "loadOperatorAvailable"
                            ]:
                                self.sendSignal(
                                    receiver=operator.candidateEntity.currentStation,
                                    signal=operator.candidateEntity.currentStation.loadOperatorAvailable,
                                )
    ...
    def findPendingObjects(self):
        ...
        for entity in G.pendingEntities:
            ...
            for machine in entity.currentStation.next:
                if machine in G.MachineList:
                    if (
                        any(type == "Load" for type in machine.multOperationTypeList)
                        and not entity.currentStation in self.pendingQueues
                    ):
        ...
        self.printTrace(
            "router found pending objects" + "-" * 6 + ">",
            [str(object.id) for object in self.pendingObjects],
        )
        self.printTrace(
            "pendingMachines" + "-" * 19 + ">",
    ...
    def findPendingEntities(self):
        ...
        for entity in G.pendingEntities:
            if (
                entity.currentStation in G.QueueList
                or entity.currentStation in G.SourceList
            ):
                for machine in entity.currentStation.next:
    ...
    def findCandidateOperators(self):
        ...
        if len(self.pending):
            ...
            for entity in [x for x in self.pending if x.manager]:
                ...
                if entity.manager.checkIfResourceIsAvailable():
                    ...
                    if (
                        entity.currentStation.canDeliver(entity)
        ...
        self.printTrace(
            "router found candidate operators" + " " * 3,
            [
                (operator.id, [station.id for station in operator.candidateStations])
                for operator in self.candidateOperators
            ],
        )
    ...
    def findCandidateReceivers(self):
        ...
        for operator in [x for x in self.candidateOperators if x.candidateEntities]:
            ...
            if operator.candidateEntity:
                if operator.candidateEntity.currentStation in self.pendingMachines:
                    operator.candidateEntity.candidateReceiver = (
                        operator.candidateEntity.currentStation
                    )
                else:
                    operator.candidateEntity.candidateReceiver = (
                        operator.candidateEntity.findCandidateReceiver()
                    )
        ...
        if len(self.conflictingEntities):
            ...
            self.conflictingOperators = [
                ...
                or operator.candidateEntity.candidateReceiver
                in [x.candidateReceiver for x in self.conflictingEntities]
            ]
        ...
        if len(self.conflictingOperators):
            ...
            for receiver in [
                x.candidateEntity.candidateReceiver for x in self.conflictingOperators
            ]:
                # find the group of operators that compete for this station
                conflictingGroup = [
                    operator
                    for operator in self.conflictingOperators
                    if operator.candidateEntity.candidateReceiver == receiver
                ]
                assert len(
                    [
                        station
                        for station in [
                            x.candidateEntity.currentStation for x in conflictingGroup
                        ]
                    ]
                ...
                for operator in conflictingGroup:
                    ...
                    operator.ind = operator.candidateEntity.currentStation.getActiveObjectQueue().index(
                        operator.candidateEntity
                    )
        ...
        self.printTrace(
            ...
            [
                (str(entity.id), str(entity.candidateReceiver.id))
                for entity in self.pending
                if entity.candidateReceiver
            ],
        )
