...
def opAss_LP(
    ...
    if weightFactors[2] > 0:
        ...
        for d in Delta_Assignment:
            obj.append(
                Delta_Ass[(d[0], d[1])]
                * (-1.0 * weightFactors[2] / (2 * len(previousAssignment)))
            )
        ...
        for d in Delta_Assignment:
            if OldAss[(d[0], d[1])] == 1:
                prob += (
                    lpSum(OldAss[(d[0], d[1])] - PB_ass[(d[0], d[1])])
                    <= Delta_Ass[(d[0], d[1])]
                )
            else:
                prob += (
                    lpSum(PB_ass[(d[0], d[1])] - OldAss[(d[0], d[1])])
                    <= Delta_Ass[(d[0], d[1])]
                )
    ...
    if weightFactors[3] > 0:
        ...
        for mach in machineList:
            if machineList[mach]["stationID"] in [0, 1, 2]:
                ...
                subline[machineList[mach]["machineID"]]["WIP"] += machineList[mach][
                    "WIP"
                ]
        ...
        if chosenSubLine:
            ...
            for station in range(3):
                mach = Tool[station][
                    chosenSubLine
                ].name  #'St'+str(station)+'_M'+str(chosenSubLine)
                for oper in PBlist:
    ...
    if LastAssignmentSum > 0 and weightFactors[4] > 0:
        obj += [
            machineList[mach]["lastAssignment"]
            * PB_ass[(oper, mach)]
            * weightFactors[4]
            / float(LastAssignmentSum)
            for oper in PBlist
            for mach in machines
            if machineList[mach]["stationID"] in PBskills[oper]
        ]
