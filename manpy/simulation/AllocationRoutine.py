...
class AllocationRoutine:
    ...
    def Run(self):
        ...
        if noItems:
            ...
            sameRouteSameWeek = Allocation(
                itemList=itemsToBeAssigned,
                week=self.week,
                altRoutes=0,
                excBuffer=self.internalBuffer,
            )
            ...
            while (
                self.week >= 0
                and (self.initialWeek - self.week) <= G.maxEarliness
                and len(self.internalBuffer)
            ):

                itemsToBeAssigned = self.internalBuffer
                self.internalBuffer = []
                sameRoutePreviousWeeks = Allocation(
                    itemList=itemsToBeAssigned,
                    week=self.week,
                    altRoutes=0,
                    excBuffer=self.internalBuffer,
            ...
            while (
                self.week < G.planningHorizon
                and (self.week - self.initialWeek) <= G.maxLateness
                and len(self.minBuffer)
            ):

                itemsToBeAssigned = self.minBuffer
                self.minBuffer = []
                sameRouteLaterWeeks = Allocation(
                    itemList=itemsToBeAssigned,
                    week=self.week,
                    altRoutes=0,
                    excBuffer=self.minBuffer,
                )
            ...
            while (
                ...
                itemsToBeAssigned = self.internalBuffer
                self.internalBuffer = []
                altRoutePreviousWeeks = Allocation(
                    itemList=itemsToBeAssigned,
                    week=self.week,
                    altRoutes=1,
                    excBuffer=self.internalBuffer,
    ...
    def filterWeek(self, buf):
        result = [
            x
            for x in buf
            if (x.originalWeek == self.week and self.itemType == x.future)
        ]
        return result
