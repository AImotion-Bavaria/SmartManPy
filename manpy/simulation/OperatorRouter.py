...
class Router(ObjectInterruption):
    ...
    def unAssignExits(self):
        ...
        for operator in [x for x in self.candidateOperators if x.isAssignedTo()]:
            if not operator.isAssignedTo() in list(
                self.pendingMachines + self.pendingQueues
            ):
                for object in [
                    x for x in operator.isAssignedTo().previous if x.exitIsAssignedTo()
                ]:
            ...
            if station:
                ...
                if operator in self.preemptiveOperators:
                    ...
                    if station != operator.workingStation:
                        ...
                        self.printTrace(
                            "router",
                            "preempting " + operator.workingStation.id + ".. " * 6,
                        )
                        operator.workingStation.preempt()
                        operator.workingStation.timeLastEntityEnded = (
                            self.env.now
                    ...
                    if station.broker.expectedSignals["resourceAvailable"]:
                        ...
                        self.printTrace(
                            ...
                            + " " * 50
                            + operator.isAssignedTo().id,
                        )
                else:
                    # signal the queue proceeding the station
                    if station.canAccept() and any(
                        type == "Load" for type in station.multOperationTypeList
                    ):
                        if station.expectedSignals["loadOperatorAvailable"]:
                            ...
                            self.printTrace(
                                "router",
                                "signalling" + " " * 50 + operator.isAssignedTo().id,
                            )
    ...
    def findPending(self):
        ...
        for entity in G.pendingEntities:
            ...
            if entity.currentStation in G.MachineList:
                if (
                    entity.currentStation.broker.waitForOperator
                    and entity.currentStation.checkIfActive()
                ):
            ...
            if entity.currentStation.getActiveObjectQueue().index(entity) == 0:
                ...
                for machine in entity.currentStation.next:
                    if (
                        machine in G.MachineList
                        and entity.checkIfRequiredPartsReady()
                        and entity.currentStation.haveToDispose()
                    ):
                        if (
                            any(
                                type == "Load" for type in machine.multOperationTypeList
                            )
    ...
    def findCandidateOperators(self):
        ...
        candidateMachines = [
            next
            for queue in self.pendingQueues
            for next in queue.findReceiversFor(queue)
        ]
                ...
                for candidateOperator in candidateOperators:
                    ...
                    if candidateOperator.skillDict:
                        if (
                            (
                                station.id
                                in candidateOperator.skillDict["process"].get(
                                    "stationIdList", []
                                )
                                and station in self.pendingMachines
                            )
                            or (
                                station.id
                                in candidateOperator.skillDict["setup"].get(
                                    "stationIdList", []
                                )
                                and station in candidateMachines
                            )
                            or (
                                (
                                    station.id
                                ...
                                and not station.getActiveObjectQueue()
                            )
                        ) and station.checkIfActive():
                            if not station in candidateOperator.candidateStations:
                                candidateOperator.candidateStations.append(station)
                    else:
                        if (
                            not station in candidateOperator.candidateStations
                            and station.checkIfActive()
                        ):
                            candidateOperator.candidateStations.append(station)
        ...
        if self.candidateOperators:
            self.printTrace(
                ...
                [
                    (
                        operator.id,
                        [station.id for station in operator.candidateStations],
                    )
                    for operator in self.candidateOperators
                ],
            )
        ...
        for operator in self.candidateOperators:
            ...
            for station in operator.candidateStations:
                if station in self.pendingMachines and not station in occupiedStations:
                    if (
                        station.currentEntity in self.pending
                        and not station.currentEntity in occupiedEntities
                    ):
                        operator.candidateEntities.append(station.currentEntity)
                else:
                    for predecessor in station.previous:
                        if (
                            predecessor in self.pendingQueues
                            and not station in occupiedStations
                            and station.isInRouteOf(predecessor)
                        ):
                            if (
                                predecessor.getActiveObjectQueue()[0] in self.pending
                                and not predecessor.getActiveObjectQueue()[0]
                                in occupiedEntities
                                and not predecessor.getActiveObjectQueue()[0]
                                in operator.candidateEntities
                            ):
                ...
                for index, entity in enumerate(operator.candidateEntities):
                    ...
                    if task_id:
                        ...
                        if requestedTechnology:
                            ...
                            if (
                                entity.currentStation in self.pendingMachines
                                and entity.schedule[-1].get("station", None)
                                == entity.currentStation
                            ):
                                # setup type (no exitTime is defined yet for the last step of the schedule)
                                if (
                                    entity.schedule[-1].get("exitTime", "None")
                                    == "None"
                                ):
                                    if requestedTechnology in operator.skillDict[
                                        "setup"
                                    ].get("technologyList", []):
                                        operator.candidateEntity = operator.candidateEntities[
                                            index
                                        ]
                                        break
                                # process type (exit time is defined for the last step of the schedule; setup step is concluded)
                                if entity.schedule[-1].get(
                                    "exitTime", "None"
                                ) != "None" or not entity.currentStep.get(
                                    "setupTime", {}
                                ):
                                    if requestedTechnology in operator.skillDict[
                                        "process"
                ...
                if operator.candidateEntity:
                    ...
                    if operator.candidateEntity.currentStation in self.pendingMachines:
                        ...
                        for station in operator.candidateStations:
                            if (
                                station
                                in operator.candidateEntity.currentStation.next
                            ):
