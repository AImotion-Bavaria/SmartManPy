...
class FutureDemandCreator:
    def run(self):
        ...
        from manpy.KnowledgeExtraction.PilotCases.InputData_DemandPlanning import (
            generateDemandPlanning,
        )

        G.generatedDemandPlanning = generateDemandPlanning(
            G.demandFile,
            PPOSToBeDisaggregated=G.TargetPPOS,
            PPOSQuantity=G.TargetPPOSqty,
            PlannedWeek=G.TargetPPOSweek,
            planningHorizon=G.maxSimTime,
            MinPackagingSize=G.minPackingSize,
        )
