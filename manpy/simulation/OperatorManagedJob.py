...
class OperatorManagedJob(Operator):
    ...
    def checkIfResourceIsAvailable(self, callerObject=None):
        ...
        if callerObject:
            ...
            #     object the operator is assigned to
                ...
                return (self.operatorAssignedTo == callerObject) and len(
                    activeResourceQueue
                ) < self.capacity
    ...
    def findCandidateEntities(self, pendingEntities=[]):
        if pendingEntities:
            for entity in [
                x
                for x in pendingEntities
                if x.currentStation.canDeliver(x) and x.manager == self
            ]:
    ...
    def findAvailableEntity(self):
        ...
        if len(
            set(self.candidateEntities).intersection(
                router.entitiesWithOccupiedReceivers
            )
        ) == len(self.candidateEntities):
            return None
        availableEntity = next(
            x
            for x in self.candidateEntities
            if not x in router.entitiesWithOccupiedReceivers
        )
