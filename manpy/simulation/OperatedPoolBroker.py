    ...
    def run(self):
        ...
        while 1:
            ...
            if self.invokeType == "request":
                # ======= request a resource
                if self.victim.isOperated() and any(
                    type == "Load" or type == "Setup" or type == "Processing"
                    ...
                    if not self.victimQueueIsEmpty():
                        ...
                        if (
                            ...
                            self.victim.printTrace(
                                self.victim.id, signal="router (broker)"
                            )
                            self.sendSignal(
                                receiver=G.RouterList[0],
                                signal=G.RouterList[0].isCalled,
                            )
                            G.RouterList[0].invoked = True

                        self.waitForOperator = True
                        self.victim.printTrace(
                            self.victim.id, waitEvent="(resourceIsAvailable broker)"
                        )
                        ...
                        self.waitForOperator = False
                        self.victim.printTrace(
                            self.victim.id, resourceAvailable="(broker)"
                        )
                        self.waitForOperator = False
                    # ===============================================================

                    assert (
                        self.victim.operatorPool.checkIfResourceIsAvailable()
                    ...
                    if not currentOperator:
                        self.victim.currentOperator = (
                            self.victim.operatorPool.findAvailableOperator()
                        )

                    with self.victim.operatorPool.getResource(
                        self.victim.currentOperator
                    ).request() as request:
                        yield request
                        ...
                        self.victim.printTrace(
                            self.victim.currentOperator.objName,
                            startWork=self.victim.id,
                        )
                        ...
                        self.victim.outputTrace(
                            self.victim.currentOperator.name,
                            "started work in " + self.victim.objName,
                        )
                        self.victim.currentOperator.timeLastOperationStarted = (
                            self.env.now
                        )  # ()
                        # signal the machine that an operator is reserved
                        if self.victim.expectedSignals["brokerIsSet"]:
                            self.sendSignal(
                                receiver=self.victim, signal=self.victim.brokerIsSet
                            )
                        # update the schedule of the operator
                        self.victim.currentOperator.schedule.append(
                            {"station": self.victim, "entranceTime": self.env.now}
                        )
                        ...
                        if self.victim.getActiveObjectQueue():
                            ...
                            if (
                                self.victim.currentOperator.schedule[-1].get(
                                    "entity", None
                                )
                                == None
                            ):
                                self.victim.currentOperator.schedule[-1][
                                    "entity"
                                ] = activeEntity
                            ...
                            try:
                                if activeEntity.currentStep:
                                    if activeEntity.currentStep.get("task_id", None):
                                        self.victim.currentOperator.schedule[-1][
                                            "task_id"
                                        ] = activeEntity.currentStep["task_id"]
