        ...
        if candidateMachines:
            ...
            for machine in candidateMachines:
                ...
                if machine.broker.waitForOperator:
                    machine.timeWaiting = (
                        self.env.now - machine.broker.timeWaitForOperatorStarted
                    )
                else:
        ...
        if self.ouputSchedule:
            ...
            for record in self.schedule:
                ...
                json["results"]["schedule"].append(
                    {"stationId": stationId, "entranceTime": record["entranceTime"]}
                )
                ...
                if record.get("entity", None):
                    ...
                    if issubclass(record["entity"].__class__, Job):
                        json["results"]["schedule"][-1]["entityId"] = record[
                            "entity"
                        ].id
