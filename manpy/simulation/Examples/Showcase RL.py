...
class ExampleEnv(QualityEnv):
    def prepare(self):
        ...
        M1 = Machine("M1", "Machine1", processingTime={"Normal": {"mean": 0.2, "stdev": 0.1, "min": 0.08, "max": 0.34}}, cost=5)
...
x = []
y = []
for i in range(0, len(simu.all_probs), len(simu.all_probs)//20):
    x.append(i/1000)
    y.append(statistics.mean([max(probs) - min(probs) for probs in simu.all_probs[i:i+len(simu.all_probs)//20]]))
