...
def controllerMethod():
    ...
    if (
        ...
        i = 0
        while (len(M1.getActiveObjectQueue()) and (not M1.state == 0)) or (
            len(M2.getActiveObjectQueue()) and (not M2.state == 0)
        ):
    ...
    while 1:
        yield G.env.timeout(0)
        if (len(M1.getActiveObjectQueue()) or M1.state == 0) and (
            len(M2.getActiveObjectQueue()) or M2.state == 0
        ):
            M1.locked = True
            M2.locked = True
            break
...
NS1 = NonStarvingEntry(
    "NS1", "Entry1", entityData={"_class": "manpy.Part", "status": "Good"}
)
NS2 = NonStarvingEntry(
    "NS2", "Entry2", entityData={"_class": "manpy.Part", "status": "Good"}
)
...
E = OpExit("E1", "Exit")
Controller = EventGenerator(
    "EV", "Controller", start=0, interval=1, method=controllerMethod
)
