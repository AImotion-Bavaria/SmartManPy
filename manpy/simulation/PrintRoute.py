...
def outputRoute():
    ...
    if G.trace == "Yes":
        if G.JobList:
            ...
            G.routeTraceSheet = G.traceFile.add_sheet(
                "sheet " + str(G.routeSheetIndex) + " route", cell_overwrite_ok=True
            )
            ...
            for j, machine in enumerate(G.MachineList):
                ...
                G.routeTraceSheet.write_merge(
                    0, 0, j * 3 + 1, j * 3 + 3, str(machine.id)
                )
