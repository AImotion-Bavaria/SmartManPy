...
class ConditionalBufferManaged(QueueManagedJob):
    ...
    def checkCondition(self):
        ...
        assert (
            activeEntity.type == "OrderComponent"
    ...
    def sortEntities(self):
        ...
        if len(managedEntities):
            ...
            activeObjectQueue.sort(
                key=lambda x: x.managerAvailable
                and (
                    x.componentType == "Basic"
                    or (x.componentType == "Secondary" and x.order.basicsEnded)
                ),
                reverse=True,
            )
        # otherwise do not check for managers availability
            ...
            activeObjectQueue.sort(
                key=lambda x: x.componentType == "Basic"
                or (x.componentType == "Secondary" and x.order.basicsEnded),
                reverse=True,
            )
    ...
    def canDeliver(self, entity=None):
        activeObject = self.getActiveObject()
        assert activeObject.isInActiveQueue(entity), (
            entity.id + " not in the internalQueue of" + activeObject.id
        )
        activeEntity = entity
        ...
        if activeEntity.type == "OrderComponent":
            if (
                activeEntity.componentType == "Secondary"
        ...
        for nextObject in [
            ...
            if object.canAcceptEntity(activeEntity)
        ]:
