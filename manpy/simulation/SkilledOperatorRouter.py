...
class SkilledRouter(Router):
    ...
    def run(self):
        ...
        while 1:
            ...
            if self.allocation:
                        ...
                        for stationId in list(self.availableStationsDict.keys()):
                            ...
                            if "Reassembly" in nextObjectClassName:
                                if nextObject.getActiveObjectQueue():
                                    if (
                                        nextObject.getActiveObjectQueue()[0].type
                                        == "Batch"
                                    ):
                                        reassemblyBlocksMachine = True
                            if machine.isBlocked or reassemblyBlocksMachine:
                                if not len(machine.getActiveObjectQueue()) and (
                                    not reassemblyBlocksMachine
                                ):
                                    raise ValueError(
                                        "empty machine considered as blocked"
                                    )
                                if (
                                    machine.timeLastEntityEnded
                                    < machine.timeLastEntityEntered
                                ):
                                    raise ValueError(
                                ...
                                if "Queue" in nextObjectClassName:
                                    if (
                                        len(nextObject.getActiveObjectQueue())
                                        != nextObject.capacity
                                    ):
                                        raise ValueError(
                                ...
                                if "Clearance" in nextObjectClassName:
                                    if not nextObject.getActiveObjectQueue():
                                        ...
                                        if (
                                            subBatchMachineHolds.parentBatch
                                            == subBatchLineClearanceHolds.parentBatch
                                            and (
                                                len(nextObject.getActiveObjectQueue())
                                                != nextObject.capacity
                                            )
                                ...
                                machinesForSecondPhaseDict[
                                    stationId
                                ] = self.availableStationsDict[stationId]
                                del self.availableStationsDict[stationId]
                            else:
                                if (
                                    len(machine.getActiveObjectQueue())
                                    and (not machine.isProcessing)
                                    and machine.onShift
                                    and machine.currentOperator
                                ):
                                    raise ValueError(
                ...
                self.solutionList.append({"time": self.env.now, "allocation": solution})
                ...
                for operatorID in list(solution.keys()):
                    ...
                    if operatorID in self.previousSolution:
                        ...
                        if (
                            not self.previousSolution[operatorID]
                            == solution[operatorID]
                        ):
                ...
                for station in toBeSignalled:
                    ...
                    if operator.workingStation:
                        ...
                        if operator.workingStation.isProcessing or (
                            not (
    ...
    def signalStation(self, station, operator):
        ...
        if station.broker.waitForOperator:
            if station.broker.expectedSignals["resourceAvailable"]:
                self.sendSignal(
                    receiver=station.broker, signal=station.broker.resourceAvailable
                )
                self.printTrace(
    ...
    def checkIfAllocationShouldBeCalled(self):
        ...
        for obj in G.OperatorsList + G.MachineList:
            ...
            if (obj.timeLastShiftEnded == self.env.now) or (
                obj.timeLastShiftStarted == self.env.now
            ):
                return True
        # loop through the machines
        for machine in G.MachineList:
            # if one machine is starved more than 30 then allocation is needed
            if (self.env.now - machine.timeLastEntityLeft >= 30) and (
                not machine.getActiveObjectQueue()
            ):
                return True
            ...
            if previousQueue:
                if (
                    float(len(previousQueue.getActiveObjectQueue()))
                    / float(previousQueue.capacity)
                    >= 0.8
                ):
                    return True
        return False
    ...
    def postProcessing(self):
        if self.outputSolutions:
            self.solutionList.append({"time": self.env.now, "allocation": {}})
    ...
    def getPreviousList(self, machine):
        ...
        while 1:
            if "Reassembly" in str(predecessor.__class__) or "Source" in str(
                predecessor.__class__
            ):
                break
    ...
    def getPreviousQueue(self, machine):
        ...
        while 1:
            if "Source" in str(predecessor.__class__):
                return None
            if "Queue" in str(predecessor.__class__) or "Clearance" in str(
                predecessor.__class__
            ):
                return predecessor
            predecessor = predecessor.previous[0]
    ...
    def getParallelMachinesList(self, machine):
        ...
        while 1:
            if (
                "Reassembly" in str(previousObject.__class__)
                or "Source" in str(previousObject.__class__)
                or "Queue" in str(previousObject.__class__)
                or "Clearance" in str(previousObject.__class__)
            ):
                break
        ...
        for nextObject in previousObject.next:
            while 1:
                if nextObject in alreadyConsidered:
                    break
                if "Machine" in str(nextObject.__class__) or "M3" in str(
                    nextObject.__class__
                ):
