...
class MachineJobShop(Machine):
    ...
    def getEntity(self):
        ...
        if activeEntity.currentStep:
            ...
            try:
                activeEntity.schedule[-1]["task_id"] = activeEntity.currentStep[
                    "task_id"
                ]
    ...
    def canAccept(self, callerObject=None):
        ...
        if self.operatorPool != "None" and (
            ...
            return (
                len(activeObjectQueue) < self.capacity
                and self.checkIfMachineIsUp()
                and self.isInRouteOf(thecaller)
                and not self.entryIsAssignedTo()
            )
    ...
    def preempt(self):
        ...
        # (may need also failure time if there was. TO BE MELIORATED)
            ...
            remainingProcessingTime = self.procTime - (
                self.env.now - self.timeLastEntityEntered
            )
        # update the remaining route of activeEntity
        activeEntity.remainingRoute.insert(
            0,
            {
                "stationIdsList": [str(self.id)],
                "processingTime": {"Fixed": {"mean": str(remainingProcessingTime)}},
            },
        )
        activeEntity.remainingRoute.insert(
            0,
            {
                "stationIdsList": [str(self.lastGiver.id)],
        ...
        self.receiver.timeLastEntityEnded = (
            self.env.now
    ...
    def checkForManualOperation(self, type, entity=None):
        ...
        assert (
            type != None
        ), "a type must be defined for the checkForManualOperation method"
        if not entity:
            activeEntity = self.getActiveObjectQueue()[0]
        else:
            activeEntity = entity
        ...
        if not self.isProcessingInitialWIP:
            ...
            operationType = activeEntity.initialOperationTypes.get(
                str(type), "not defined"
            )
