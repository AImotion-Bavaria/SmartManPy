...
class CapacityStationController(EventGenerator):
    def __init__(
        ...
        method=None,
        argumentDict={},
        dueDateThreshold=float("inf"),
        prioritizeIfCanFinish=False,
        **kw
    ):
        EventGenerator.__init__(
            self, id, name, start, stop, interval, duration, method, argumentDict
        )
        ...
        if hasattr(G, "extraPropertyDict"):
            self.assemblySpace = float(
                G.extraPropertyDict.get("assemblySpace", float("inf"))
            )
        else:
    ...
    def steps(self):
        ...
        for station in G.CapacityStationList:
            ...
            for entity in entitiesToCheck:
                if (
                    not exit.isRequested.triggered
        ...
        for station in G.CapacityStationList:
            ...
            for entity in entitiesToCheck:
                ...
                station.detailedWorkPlan.append(
                    {
                        "time": self.env.now,
                ...
                if self.checkIfProjectJustStartsInStation(project, station):
                    project.projectSchedule.append(
                        {"stationId": station.id, "entranceTime": self.env.now}
                    )
        ...
        for station in G.CapacityStationList:
            ...
            if str((self.env.now) + 1.0) in list(
                station.intervalCapacityExceptions.keys()
            ):
                station.remainingIntervalCapacity[
                    0
                ] = station.intervalCapacityExceptions[str((self.env.now) + 1)]
        ...
        for buffer in G.CapacityStationBufferList:
            ...
            while not allCapacityConsumed:
                ...
                for entity in entitiesNotAllocated:
                    ...
                    if (
                        self.checkIfProjectCanStartInStation(
                ...
                for entity in entitiesWithinThreshold:
                    ...
                    if (
                        ...
                        if not self.checkIfProjectConsumesAssemblySpace(
                            ...
                            consideredSpace -= (
                                entity.capacityProject.assemblySpaceRequirement
                            )
                ...
                if totalRequestedCapacity <= totalAvailableCapacity:
                    ...
                    for entity in entitiesWithinThreshold:
                        ...
                        if (
                            ...
                            and entity.requiredCapacity <= availableCapacity
                        ):
                            ...
                            if entityBuffer.requireFullProject and (
                                not self.checkIfProjectConsumesAssemblySpace(
                                    entity, entityBuffer
                                )
                            ):
                                availableSpace -= (
                ...
                # else calculate the capacity for every entity and create the entities
                    ...
                    for e in entitiesToBeBroken:
                        ...
                        if (
                            self.checkIfAProjectCanBeFinishedInStation(
                            ...
                            and self.checkIfProjectCanStartInStation(
                                e.capacityProject, e.currentStation.next[0]
                            )
                            and self.checkIfThereIsEnoughSpace(
                    ...
                    entitiesToBeBroken.sort(
                        key=lambda x: x.willFinishNow and self.prioritizeIfCanFinish,
                        reverse=True,
                    )
                    ...
                    for entity in entitiesToBeBroken:
                        ...
                        if (
                            ...
                            if (
                                self.checkIfAProjectCanBeFinishedInStation(
                                    entity, entityStation, totalAvailableCapacity
                                )
    ...
    def breakEntity(
        self, entity, buffer, station, totalAvailableCapacity, totalRequestedCapacity
    ):
    ...
    def checkIfAProjectCanBeFinishedInStation(self, entity, station, availableCapacity):
        ...
        if (
            total - (alreadyWorked + required) < 0.001 and availableCapacity >= required
    ...
    def sortBuffers(self):
        for buffer in G.CapacityStationBufferList:
            ...
            buffer.sharedPriority = station.sharedResources.get(
                "priority", -float("inf")
            )
